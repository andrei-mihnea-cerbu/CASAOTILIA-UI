...
const CreateCarDialog: React.FC<Props> = ({ open, onClose, onCreated }) => {
  const [formData, setFormData] = useState<
    Omit<Car, 'id' | 'imageGallery' | 'discount' | 'salePrice'>
  >({
    name: '',
    year: new Date().getFullYear(),
    price: 0,
    engine: '',
    stock: 'YES' as StockStatus,
    gearType: 'MANUAL' as GearType,
    gasType: 'PETROL' as GasType,
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await axios.post(`${apiUrl}/cars`, payload, {
        headers: getAuthHeaders(),
      });

      onCreated();
      setNotification('Car created successfully.', 'success');
      onClose();
    } catch (error: any) {
      setNotification(
        error?.response?.data?.message || 'Failed to create car.',
        'error'
      );
    } finally {
      setLoading(false);
